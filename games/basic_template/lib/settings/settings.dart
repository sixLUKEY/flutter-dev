...
class SettingsController {
  ...
  // The player's name, used for things like high-score list
  ValueNotifier<String> playerName = ValueNotifier('Player');

  // Whether or not the Sfx are on
  ...
  // (i.e. NSUserDefaults on iOS, SharedPreferences on Android or local storage on the web)
  ...
  Future<void> _loadStateFromPersistence() async {
    final loadedValues = await Future.wait([
      _store.getAudioOn(defaultValue: true).then((value) {
        if (kIsWeb) {
          // On the web, sounds can only start after user interaction, so we start
          // muted on every game start.
